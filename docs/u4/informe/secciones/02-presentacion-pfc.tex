...
(\texttt{Rol}, columna \url{usuarios.rol}): \texttt{ROLE\_ADMIN},
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lp{8cm}@{}}
    ...
    \midrule
    Backend & Java 21, Spring Boot 3.2.12~\parencite{spring-boot-docs} \\
    Seguridad & Spring Security 6, JWT (\texttt{jjwt} 0.12.6, HMAC-SHA256)~\parencite{rfc7519} \\
    Persistencia & Spring Data JPA + Hibernate, PostgreSQL 16, Flyway \\
    Cach\'e / revocaci\'on & Redis 7 (Spring Data Redis + Spring Cache) \\
    Documentaci\'on de API & springdoc-openapi 2.5.0 (Swagger UI)~\parencite{openapi-spec} \\
    Cobertura & JaCoCo 0.8.12 \\
    Frontend & Angular 17.3, TypeScript 5.4, componentes standalone~\parencite{angular-docs} \\
    Manejo uniforme de errores & \texttt{ProblemDetail} (RFC 7807)~\parencite{rfc7807} \\
    Orquestaci\'on & Docker Compose, \texttt{Makefile} \\
    \bottomrule
  \end{tabular}
\end{table}

\evidencia[0.62\textwidth]{../../diagrams/c4-contenedores/c4-contenedores.png}{Diagrama C4 Nivel 2 (contenedores) de BIOPET.}{fig:c4-contenedores}

El detalle completo de la arquitectura, con relaciones verificadas contra
el c\'odigo, sigue el modelo C4~\parencite{c4-model} y est\'a documentado en
