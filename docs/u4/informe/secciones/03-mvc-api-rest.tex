...
\texttt{ProblemDetail} (RFC 7807)~\cite{rfc7807}, y todos los recursos
...
especificaci\'on OpenAPI~\cite{openapi-spec}. La configuraci\'on real
(\texttt{Backend/src/main/resources/application.yml}) expone:
...
\texttt{SwaggerUiTest}: la primera prueba confirma que \texttt{/api/docs}
redirige a la interfaz real de Swagger UI (cuyo cuerpo contiene el
marcador \texttt{swagger-ui}); la segunda confirma que \texttt{/api/openapi}
