...
| Transformación aplicada | **Única transformación declarada:** sustitución textual del identificador de usuario del sistema operativo en rutas locales embebidas por la herramienta (`<perfil-de-usuario>/...` → `USER_REDACTED`), documentada explícitamente en `README.md`. Puntajes, fechas, URLs y assertions **no fueron alterados** |
