...
\textbf{Nota sobre el renderizado:} a la fecha de este informe no exist\'ia
en el entorno de trabajo un renderizador local de Graphviz ni de PlantUML,
por lo que la fuente \texttt{c4-componentes-backend.dot} a\'un no cuenta con
una imagen \texttt{.png} generada (validada \'unicamente de forma manual:
balance de llaves/corchetes/comillas y verificaci\'on cruzada de que cada
arista referencia un nodo declarado). Cuando se genere esa imagen, debe
copiarse a \texttt{figuras/jaime/c4-componentes-backend.png} para que
aparezca autom\'aticamente en este informe.
