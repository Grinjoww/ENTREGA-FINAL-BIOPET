...
corridas ($r$ desde $-0{,}25$ hasta $-0{,}85$; corregido por comparaciones
m\'ultiples con Holm--Bonferroni, secci\'on~\ref{sec:rendimiento-final}),
lo que sugiere que factores
...
Retomando las preguntas de investigaci\'on planteadas en la
secci\'on~\ref{sec:preguntas-investigacion} del cap\'itulo de
introducci\'on (cap\'itulo~\ref{cap:introduccion}), esta secci\'on
responde cada una con la evidencia emp\'irica recabada.
...
declarados: el tag Git \texttt{v1.0.0} existe (creado originalmente sobre
el commit hist\'orico \texttt{0d5cd52}, 2026-08-18, y actualizado al
cierre de esta recalificaci\'on para identificar el commit final
evaluado --- ver secci\'on~\ref{sec:pendientes-honestos-publicacion}),
pero el GitHub Release correspondiente todav\'ia no ha
sido publicado en la p\'agina \emph{Releases} del repositorio --- ambos
artefactos son distintos y no deben confundirse. Adem\'as, el plan
gratuito de PostgreSQL en Render no permite verificar hoy una
