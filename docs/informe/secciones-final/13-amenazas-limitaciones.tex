...
\begin{itemize}[nosep]
  ...
  \item La muestra SUS ($n=18$, reclutada por conveniencia dentro del
    c\'irculo cercano al equipo) no es necesariamente representativa de
    usuarios finales de una cl\'inica veterinaria real; un tama\~no
    muestral mayor reduce el margen del intervalo de confianza pero no
    cambia el m\'etodo de reclutamiento. Justificaci\'on completa de
    reclutamiento, poblaci\'on de origen, sesgos introducidos y l\'imites
    de generalizaci\'on, con respaldo en Ralph, Baltes et al.
    (2021)~\cite{ralph2021-empirical-standards}, en la
    secci\'on~\ref{sec:muestreo-sus-metodologia}.
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Tag \texttt{v1.0.0} y GitHub Release: contexto hist\'orico
    y estado actual.} En el momento en que se redact\'o originalmente esta
    limitaci\'on, ni el tag Git \texttt{v1.0.0} ni un GitHub Release
    exist\'ian todav\'ia, por quedar ambos expl\'icitamente fuera del
    alcance de esa fase de redacci\'on del informe. Esa parte de la
    limitaci\'on qued\'o \textbf{resuelta solo parcialmente} y de forma
    distinta para cada artefacto, que \textbf{no deben confundirse}:
    \begin{itemize}[nosep]
      \item \textbf{Tag Git \texttt{v1.0.0}:} ya fue creado y publicado
        originalmente el 2026-08-18 sobre el commit hist\'orico
        \texttt{0d5cd52}, cierre de la Entrega Final original. Conforme a
        la r\'ubrica de esta recalificaci\'on, ese mismo tag se actualiza
        para identificar el commit final evaluado al cierre del examen,
        sin cambiar su nombre ni crear un tag adicional (ver
        secci\'on~\ref{sec:pendientes-honestos-publicacion}).
      \item \textbf{GitHub Release:} a la fecha de cierre de este
        documento \textbf{sigue sin publicarse} en la p\'agina
        \emph{Releases} del repositorio. La existencia del tag Git no
        implica autom\'aticamente un GitHub Release publicado; son dos
        artefactos distintos de GitHub, y esta limitaci\'on permanece
        vigente para el segundo.
    \end{itemize}
    (Ver secci\'on~\ref{sec:pendientes-honestos-publicacion} para el
    detalle t\'ecnico.)
\end{itemize}
