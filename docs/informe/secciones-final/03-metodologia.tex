...
La tabla~\ref{tab:peffers} mapea cada una de las seis actividades del
marco de Peffers et al.~\cite{peffers2007dsrm} sobre la evidencia real
producida por BIOPET; el cumplimiento del est\'andar Engineering Research
correspondiente se audita en el anexo~E
(secci\'on~\ref{sec:anexo-ralph}).
...
\begin{table}[H]
  ...
  \label{tab:peffers}
  \begin{tabular}{@{}p{3.2cm}p{2.0cm}p{7.4cm}@{}}
    \toprule
    Actividad & Estado & Evidencia \\
    \midrule
    1. Identificaci\'on del problema y motivaci\'on & Completada & 15 observaciones docentes reales registradas y cerradas (\texttt{docs/observaciones/OBSERVACIONES.md}, detalladas en el anexo~A, secci\'on~\ref{sec:anexo-observaciones}); SRS \S1 \\
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{2.0cm}p{4.6cm}p{4.6cm}@{}}
    ...
    Mantenibilidad & \textquestiondown El proceso de build es reproducible y automatizado? & CI con 6 jobs verdes (\texttt{.github/workflows/ci.yml}, anexo~D, secci\'on~\ref{sec:anexo-ci}) \\
    ...
    Seguridad (din\'amica) & \textquestiondown Hay hallazgos de severidad alta por escaneo din\'amico? & 0 alertas altas; 2 tipos informativos (3 instancias); escaneo autenticado local TLS (\texttt{docs/mediciones/sec/zap/README.md}) \\
...
Para cada pregunta GQM definida en la secci\'on~\ref{sec:gqm} y resumida en la tabla~\ref{tab:gqm} se documenta prop\'osito,
unidad evaluada, procedimiento reproducible, m\'etrica principal y
limitaci\'on conocida (detalle ampliado por dimensi\'on en los
...
(secci\'on~\ref{sec:sus-final}), aplicada con el instrumento original de
Brooke (1996) reproducido en el anexo~C
(secci\'on~\ref{sec:anexo-sus}), se reclutaron por \textbf{conveniencia,
