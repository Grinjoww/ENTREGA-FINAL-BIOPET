...
sobre el commit hist\'orico exacto \texttt{0d5cd52} (cierre de la Entrega
Final original, al que apunt\'o el tag \texttt{v1.0.0} hasta el cierre de
esta recalificaci\'on) y verificada directamente contra
\texttt{Backend/target/surefire-reports/} de esa reproducci\'on
(\texttt{docs/mediciones/sec/reproduccion-v1.0.0/}), reporta el resultado
que resume la tabla~\ref{tab:pruebas-final}:

\begin{table}[H]
  \centering
  \caption{Resultado real de la suite de pruebas del backend sobre el commit hist\'orico \texttt{0d5cd52} (evidencia hist\'orica; ver estado del \texttt{HEAD} actual m\'as abajo).}
...
{Resultado real de \texttt{mvn clean verify} sobre el commit hist\'orico \texttt{0d5cd52}: 205 pruebas, 0 fallos, 0 errores, 0 omitidas, \texttt{BUILD SUCCESS} (regenerada por \texttt{docs/informe/scripts/generar-figuras-evidencia.py} desde el log crudo versionado).}%
{fig:maven-verify-final}

Estas 205 pruebas corresponden al estado hist\'orico del commit
\texttt{0d5cd52} (cierre de la Entrega Final original) y constituyen la
\'unica ejecuci\'on reproducible archivada en este repositorio: 205
pruebas, 0 fallos, 0 errores y 0 omitidas, con el log crudo de
Maven/Surefire versionado. El \texttt{HEAD} actual de la rama de
recalificaci\'on incorpora pruebas nuevas sobre esa misma base, sin
sustituirla, y contiene un \textbf{inventario est\'atico de 217 anotaciones
\texttt{@Test} distribuidas en 24 clases de prueba},
contabilizables en cualquier \emph{checkout} sobre
\rutalarga{Backend/src/test/java/com/biopet/}. Esa cifra describe el
inventario actual del c\'odigo y \textbf{no debe interpretarse como una
corrida reproducible archivada}: para el \texttt{HEAD} no se conserva un
log de Maven/Surefire equivalente al de \texttt{0d5cd52}, por lo que no
se le atribuye aqu\'i ning\'un resultado de ejecuci\'on ni
\texttt{BUILD SUCCESS}. Ambas cifras son correctas dentro de su alcance
respectivo --- 205 como resultado de ejecuci\'on hist\'orica verificada,
217 como conteo est\'atico vigente ---, y no son magnitudes
homologables entre s\'i. Detalle de la relaci\'on
(205~$\rightarrow$~217) y procedencia de cada estado en
...
(Tercera Entrega) a \textbf{70\,\%} para v1.0.0. La
tabla~\ref{tab:jacoco-final} resume el resultado medido.

\begin{table}[H]
  \centering
  \caption{Cobertura medida por \texttt{jacoco:check} sobre el commit hist\'orico \texttt{0d5cd52} (evidencia hist\'orica; fuente: \texttt{docs/mediciones/jacoco/METRICS.md}). Ver estado del \texttt{HEAD} actual m\'as abajo.}
...
estado hist\'orico del commit \texttt{0d5cd52} (Entrega Final original).
Recalculada sobre
...
de 30~s por corrida. La tabla~\ref{tab:k6-final} detalla el resultado de
las diez corridas.

\begin{table}[H]
  \centering
  \tiny
  \caption[Resultado real de las diez corridas de k6.]{Resultado real de las diez corridas de k6 (fuente: \texttt{docs/mediciones/perf/REPORT.md}).}
  \label{tab:k6-final}
  \begin{tabular}{@{}lrrrrrrr@{}}
    \toprule
    Corrida & n & Media (ms) & Mediana (ms) & p95 (ms) & p99 (ms) & Error (\%) & Throughput (req/s) \\
    \midrule
    caliente-01 & 3217 & 8{,}66 & 7{,}59 & 15{,}06 & 20{,}56 & 0{,}0 & 91{,}83 \\
    caliente-02 & 3236 & 7{,}00 & 6{,}68 & 9{,}48 & 13{,}06 & 0{,}0 & 92{,}35 \\
    caliente-03 & 3236 & 5{,}48 & 5{,}27 & 6{,}94 & 9{,}38 & 0{,}0 & 92{,}48 \\
    caliente-04 & 3236 & 4{,}92 & 4{,}75 & 6{,}04 & 8{,}79 & 0{,}0 & 92{,}62 \\
    caliente-05 & 3246 & 5{,}04 & 4{,}89 & 6{,}48 & 7{,}68 & 0{,}0 & 92{,}61 \\
    fr\'io-01 & 3211 & 9{,}33 & 8{,}06 & 15{,}31 & 21{,}10 & 0{,}0 & 91{,}65 \\
    fr\'io-02 & 3216 & 9{,}09 & 7{,}82 & 15{,}13 & 19{,}14 & 0{,}0 & 91{,}86 \\
    fr\'io-03 & 3210 & 10{,}26 & 8{,}37 & 18{,}23 & 25{,}89 & 0{,}0 & 91{,}57 \\
    fr\'io-04 & 3211 & 9{,}88 & 7{,}84 & 18{,}56 & 26{,}05 & 0{,}0 & 91{,}57 \\
    fr\'io-05 & 3207 & 10{,}13 & 8{,}54 & 18{,}03 & 25{,}97 & 0{,}0 & 91{,}52 \\
    \bottomrule
  \end{tabular}
\end{table}
...
por ejemplo, caliente-01: $[8{,}40, 8{,}93]$~ms. Adem\'as del IC~95\,\%,
...
Rosenthal: el efecto es moderado en la corrida~01 ($r=-0{,}25$) y grande en
las corridas~03--05 ($r=-0{,}83$ a $-0{,}85$), consistente con una cach\'e
que reduce la latencia de forma creciente conforme el sistema se estabiliza
entre corridas consecutivas. La figura~\ref{fig:performance-report-final}
muestra la vista agregada de este reporte, y las
figuras~\ref{fig:rendimiento-caliente-final}
y~\ref{fig:rendimiento-frio-final} muestran la salida de consola real de
una corrida representativa en cada condici\'on.

\textbf{Correcci\'on por comparaciones m\'ultiples (Holm--Bonferroni).}
Las cinco corridas fr\'io--caliente (01 a 05) no son cinco repeticiones
independientes de una \'unica pregunta, sino \textbf{una familia de $m=5$
contrastes} (un Wilcoxon pareado por corrida) evaluados sobre el mismo
conjunto de datos de rendimiento; contrastar los cinco sin ajuste
infla el error de tipo~I familiar por encima del $\alpha=0{,}05$ nominal.
\texttt{docs/mediciones/perf/REPORT.md} aplica el m\'etodo secuencial de
\textbf{Holm--Bonferroni} ($\alpha=0{,}05$, $m=5$) sobre los $p$ crudos del
Wilcoxon pareado, reproducible en el propio reporte. La
tabla~\ref{tab:holm-bonferroni-k6} reproduce los cinco contrastes con su
$p$ crudo y su $p$ ajustado por Holm.

\begin{table}[H]
  \centering
  \small
  \caption{Correcci\'on Holm--Bonferroni sobre los cinco contrastes Wilcoxon pareados fr\'io vs.\ caliente (fuente: \texttt{docs/mediciones/perf/REPORT.md}).}
  \label{tab:holm-bonferroni-k6}
  \begin{tabular}{@{}lllll@{}}
    \toprule
    Contraste & $p$ crudo & $p$ ajustado (Holm) & \'Signif. ($\alpha=0{,}05$) & $r$ (Rosenthal) \\
    \midrule
    01 & $1{,}10\times10^{-45}$ & $1{,}10\times10^{-45}$ & S\'i & $-0{,}25$ \\
    02 & $3{,}88\times10^{-155}$ & $7{,}75\times10^{-155}$ & S\'i & $-0{,}47$ \\
    03 & $p<2{,}23\times10^{-308}$ & $p<2{,}23\times10^{-308}$ & S\'i & $-0{,}83$ \\
    04 & $p<2{,}23\times10^{-308}$ & $p<2{,}23\times10^{-308}$ & S\'i & $-0{,}85$ \\
    05 & $p<2{,}23\times10^{-308}$ & $p<2{,}23\times10^{-308}$ & S\'i & $-0{,}85$ \\
    \bottomrule
  \end{tabular}
\end{table}

Con Holm--Bonferroni, cada $p$ crudo se compara contra un umbral
progresivamente menos estricto seg\'un su orden (el m\'as peque\~no contra
$\alpha/m$, el siguiente contra $\alpha/(m-1)$, y as\'i sucesivamente),
en vez de comparar los cinco contra el mismo $\alpha/m$ fijo de Bonferroni
cl\'asico --- por eso el $p$ ajustado de la corrida~01 no cambia
respecto al crudo (ya era, por s\'i solo, varios \'ordenes de magnitud
menor que $\alpha/m=0{,}01$), mientras que el de la corrida~02 s\'i se
duplica. Las cinco corridas permanecen \textbf{significativas despu\'es
del ajuste} ($p$ ajustado $\ll 0{,}05$ en los cinco casos): la
correcci\'on por comparaciones m\'ultiples \textbf{no cambia la
conclusi\'on} de que la condici\'on fr\'ia produce latencias
sistem\'aticamente mayores que la caliente en las cinco corridas. Esto
\textbf{no implica} que la cach\'e sea la \'unica causa posible de la
diferencia observada --- ver la nota metodol\'ogica sobre pareo por
\'indice de llegada m\'as abajo y el an\'alisis de sensibilidad Mann--Whitney
en el mismo \texttt{REPORT.md} --- \'unicamente que, bajo el dise\~no y los
datos reales de esta medici\'on, el efecto no es un artefacto de comparar
cinco veces sin control del error familiar.
...
(\rutalarga{docs/etica/regularizacion-sus/CONSTANCIA-REGULARIZACION-SUS-BIOPET-2026-08-31.pdf}),
que \textbf{no sustituye} consentimientos individuales. La
tabla~\ref{tab:sus-final} resume los estad\'isticos agregados.
...
(\texttt{lighthouserc.js}\allowbreak/\texttt{lighthouserc.desktop.js}, tambi\'en
...
el \textbf{2026-09-03}
(\texttt{lighthouserc.render.js}\allowbreak/\texttt{lighthouserc.render.desktop.js},
\rutalarga{docs/mediciones/lighthouse/lhci-20260903-2102-render-*.json},
metadatos en \texttt{lhci-20260903-2102-\allowbreak render.meta.txt}; metodolog\'ia y
medias reproducibles en \rutalarga{docs/mediciones/lighthouse/RENDER-REPORT.md}),
la primera de las tres auditando \texttt{requestedUrl} p\'ublico
(\rutalarga{https://biopet-frontend.onrender.com}) en vez de \texttt{localhost}.
...
contra el dominio p\'ublico antes de archivarlo. La
tabla~\ref{tab:lighthouse-final} resume las medias por categor\'ia, perfil
y ruta.
...
La figura~\ref{fig:lighthouse-final} muestra la vista del reporte
Lighthouse~CI correspondiente.
...
sobre el \texttt{BUNDLE} completo, secci\'on~\ref{sec:jacoco-final}), usabilidad (SUS, ejecutada, $n=18$),
