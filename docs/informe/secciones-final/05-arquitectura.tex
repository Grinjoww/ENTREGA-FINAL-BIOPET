...
\texttt{docs/diagrams/} (modelo C4~\cite{c4-model}, con
\texttt{docs/diagrams/workspace.dsl} como fuente Structurizr DSL \'unica de
los tres niveles) y las decisiones de arquitectura relevantes
(\texttt{docs/adr/}), actualizada para reflejar el estado v1.0.0
(PostgreSQL~18/Valkey~8 en producci\'on, acceso JPA formal a procedimientos
almacenados).
...
\label{sec:c4-nivel-1}

El diagrama de contexto (\texttt{docs/diagrams/c4-contexto/},
figura~\ref{fig:c4-nivel-1-final}) sit\'ua a BIOPET como una \'unica caja
de software, frente a los cuatro roles reales
del sistema (\texttt{ROLE\_ADMIN}, \texttt{ROLE\_VETERINARIO},
\texttt{ROLE\_AUXILIAR}, \texttt{ROLE\_DUENO}) y al \'unico sistema externo
real de v1.0.0: \emph{API Ninjas (Animals API)}, consumido desde
\texttt{ExternalApiController}/\texttt{ExternalApiService} con cach\'e de
respuesta en Redis/Valkey.

% EVIDENCIA-COMPARTIDA-C4-L1
% Ruta esperada: figuras/compartidas/c4-contexto.png
\evidencia{figuras/compartidas/c4-contexto.png}%
{Diagrama C4 Nivel 1 (contexto) de BIOPET: actores por rol y el sistema externo API Ninjas.}%
{fig:c4-nivel-1-final}

\section{C4 Nivel 2 --- Contenedores}
\label{sec:c4-nivel-2}

A un nivel de detalle mayor que el de contexto
(secci\'on~\ref{sec:c4-nivel-1}), el diagrama de contenedores
(\texttt{docs/diagrams/c4-contenedores/},
figura~\ref{fig:c4-nivel-2-final}) describe los cuatro contenedores del
sistema: el frontend Angular, el backend Spring Boot, PostgreSQL y
Redis/Valkey, junto con los protocolos reales entre ellos (HTTPS/JSON con
cookies entre frontend y backend; JDBC entre backend y PostgreSQL; el
cliente Redis de Spring Data entre backend y Redis/Valkey).
...
El diagrama de componentes del backend (figura~\ref{fig:c4-nivel-3-final}), que detalla los contenedores descritos en la secci\'on~\ref{sec:c4-nivel-2},
fue elaborado mediante inspecci\'on directa del c\'odigo fuente
(constructores, llamadas directas, configuraci\'on de Spring), no por
convenci\'on de nombres. A los
...
(figura~\ref{fig:sp-acceso-hibrido-final}), en vez de
\texttt{@Query(nativeQuery=true)} plano (usado en la Tercera Entrega solo
para la primera rutina). Las cuatro rutinas con prefijo \texttt{fn\_*}
originalmente eran \texttt{FUNCTION}; se reclasificaron a
\texttt{PROCEDURE} (conservando el nombre por compatibilidad) para permitir
esa invocaci\'on formal, formalizada como ADR-007 (secci\'on~\ref{sec:adr-relevantes-final}).
La tabla~\ref{tab:catalogo-sp} cataloga las seis rutinas resultantes.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{4.0cm}p{2.35cm}p{6.45cm}@{}}
    \toprule
...
\texttt{V6} sobre una base que ya tenga \texttt{V1--V5} aplicadas (detalle
completo de las seis migraciones en la
secci\'on~\ref{sec:migraciones-flyway}). La invocaci\'on formal resultante
se verific\'o adem\'as de forma manual
contra una rutina almacenada real desde Postman
(figura~\ref{fig:postman-sp-final}).
...
La tabla~\ref{tab:adr-relevantes-final} resume las decisiones de
arquitectura vigentes en \texttt{docs/adr/}, referenciadas a lo largo de
este cap\'itulo:
...
all\'a de la configuraci\'on externalizada por variables de entorno. La
tabla~\ref{tab:iso25010-resumen} resume ese mapeo.
