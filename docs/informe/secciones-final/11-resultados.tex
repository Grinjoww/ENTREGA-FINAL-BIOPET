...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{3.3cm}p{5.3cm}p{4.2cm}@{}}
    \toprule
    Dimensi\'on & Resultado & Fuente \\
    \midrule
    Pruebas automatizadas & 205 pruebas, 0 fallos, 0 errores & \rutalarga{surefire-reports/} \\
    Cobertura (JaCoCo) & LINE 91{,}80\,\%, BRANCH 79{,}39\,\%, umbral 70\,\% & \rutalarga{docs/mediciones/jacoco/METRICS.md} \\
    Rendimiento (k6) & 10 corridas (5 fr\'io + 5 caliente), error 0{,}0\,\% & \rutalarga{docs/mediciones/perf/REPORT.md} \\
    Usabilidad (SUS) & Media 74{,}44/100, IC95\,\% $[63{,}33,85{,}56]$, n=18 & \rutalarga{docs/mediciones/sus/REPORT.md} \\
    Calidad web (Lighthouse) & 4/4 categor\'ias cumplen umbral (SEO 100/100, mobile+desktop) & \rutalarga{docs/mediciones/lighthouse/} \\
    Seguridad OWASP (manual) & 6 controles auditados, todos \texttt{PASS} & \rutalarga{docs/mediciones/sec/REPORT.md} \\
    Seguridad din\'amica (ZAP) & Escaneo autenticado local TLS (evidencia vigente; complementa el \emph{baseline} hist\'orico sin autenticar, cap\'itulo~\ref{cap:seguridad}): 0 altas/medias/bajas; 2 tipos informativos (3 instancias); rutas protegidas verificadas en \texttt{2xx} & \rutalarga{docs/mediciones/sec/zap/README.md} \\
    ...
    Auditor\'ia SQL din\'amico & 0 hallazgos en \rutalarga{db/procs/} & \rutalarga{scripts/audit-sql-dynamic.sh} \\
    CI & 6 jobs verdes & \rutalarga{.github/workflows/ci.yml} \\
    Despliegue & Frontend y backend en Render, HTTPS activo, \rutalarga{/actuator/health} \texttt{UP} & \rutalarga{docs/despliegue/DEPLOYMENT.md} \\
