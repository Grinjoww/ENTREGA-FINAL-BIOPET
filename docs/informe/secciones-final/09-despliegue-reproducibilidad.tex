...
con HTTPS autom\'atico sobre subdominios \texttt{*.onrender.com}, seg\'un
resume la tabla~\ref{tab:produccion-final}.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{3.0cm}p{11.5cm}@{}}
    ...
    Servicio & URL / valor \\
    \midrule
    Frontend & \rutalarga{https://biopet-frontend.onrender.com} \\
    Backend & \rutalarga{https://biopet-backend-dh5e.onrender.com} \\
    \emph{Healthcheck} & \rutalarga{GET /actuator/health} $\to$ \texttt{\{"status":\allowbreak"UP","groups":\allowbreak["liveness","readiness"]\}} \\
...
\texttt{.github/workflows/ci.yml} define seis \emph{jobs}
(tabla~\ref{tab:ci-jobs}), todos en verde a la fecha de cierre de esta
entrega:
...
Registry (tabla~\ref{tab:ghcr-final}):
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{3.6cm}p{10.9cm}@{}}
    ...
    Campo & Valor \\
    \midrule
    Imagen & \rutalarga{ghcr.io/grinjoww/entregafinal-biopet-backend} \\
    Tag publicado & \texttt{sha-fe2f033} \\
    Digest (\texttt{sha256}, inmutable) & \texttt{sha256:ef1e857a95a307a1\allowbreak 15ebe01599a41506\allowbreak eab824808b70a3c8\allowbreak e317dcc55bef5163} \\
...
El tag Git \texttt{v1.0.0} ya fue creado y publicado (identifica el
commit evaluado al cierre de esta entrega/recalificaci\'on; ver
...
est\'an archivados en Zenodo con DOI reales (tabla~\ref{tab:zenodo-final}),
propagados a \texttt{CITATION.cff}, \texttt{README.md} y
\texttt{docs/checklists/fair.md}:
...
\section{Estado de publicaci\'on del tag}
\label{sec:pendientes-honestos-publicacion}

El tag Git \texttt{v1.0.0} \textbf{ya existe y ya fue publicado}
(verificable con \texttt{git rev-list -n 1 v1.0.0}). Se cre\'o
originalmente el 2026-08-18 sobre el commit hist\'orico \texttt{0d5cd52},
registrando el cierre de la Entrega Final original. Conforme a la
r\'ubrica de esta recalificaci\'on -- que eval\'ua el commit al que
apunte \texttt{v1.0.0} al cierre del examen final --, ese mismo tag se
actualiza (\texttt{git tag -f}) para identificar el commit final de esta
rama de correcciones, sin cambiar su nombre (\texttt{v1.0.0}) ni crear un
tag adicional. El commit hist\'orico \texttt{0d5cd52} permanece
documentado como tal (por ejemplo, en las cifras de pruebas y cobertura
de la Entrega Final original citadas en el cap\'itulo~\ref{cap:pruebas-calidad}),
aunque el tag ya no apunte a \'el una vez movido. A la fecha de
cierre de este documento:

\begin{itemize}[nosep]
  \item El tag Git \texttt{v1.0.0} existe en el repositorio.
...
\begin{itemize}[nosep]
  \item \textbf{Arranque completo:} \texttt{make up} (o \texttt{docker
    compose up --build -d}), sin pasos manuales adicionales, con los
    servicios reales en estado \texttt{healthy}
    (figura~\ref{fig:docker-healthy-final}).
  \item \textbf{Im\'agenes de terceros fijadas por \emph{digest}
    \texttt{sha256}} (no solo \emph{tag}), para evitar derivas silenciosas
    entre reconstrucciones (figura~\ref{fig:docker-digest-final}).
