...
La tabla~\ref{tab:glosario-final} recoge las siglas usadas a lo largo de
este informe.
...
La tabla~\ref{tab:endpoints-final} lista los endpoints reales expuestos
por el backend v1.0.0.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{3.0cm}p{4.2cm}p{7.0cm}@{}}
    \toprule
    ...
    GET\allowbreak/POST\allowbreak/PUT\allowbreak/DELETE & /api\allowbreak/mascotas[\allowbreak/\{id\}] & Rol y propiedad (secci\'on \ref{cap:seguridad}) \\
...
\begin{itemize}[nosep]
  ...
  \item Checklists de rigor metodol\'ogico: \rutalarga{docs/checklists/} (\texttt{ralph2021-\allowbreak engineering-\allowbreak research.md}, \texttt{prisma2020.md}, \texttt{incose2023-req.md}, \texttt{fair.md}).
...
\small
\begin{itemize}[nosep]
  \item \texttt{AuthControllerTest}, \texttt{JwtCookie\allowbreak AuthenticationTest},
    \texttt{JwtServiceTest}, \texttt{LoginRate\allowbreak LimiterServiceTest},
    \texttt{Authentication\allowbreak AuditServiceTest} (seguridad y autenticaci\'on).
  ...
  \item \texttt{SqlInjectionSecurityTest}, \texttt{SecurityHeadersTest}
    (OWASP A03 y A05).
  \item \texttt{Resumen\allowbreak Especies\allowbreak IntegrationTest} (Testcontainers, PostgreSQL real).
\end{itemize}
\normalsize
...
\texttt{docs/observaciones/OBSERVACIONES.md}; la
tabla~\ref{tab:anexo-observaciones} detalla cada una.
...
\rutalarga{docs/checklists/prisma2020.md} (\'item~7); la versi\'on completa
por bloque, con fuentes y ventana temporal, est\'a en la
secci\'on~\ref{sec:cadena-busqueda-trabajos-relacionados} del
cap\'itulo de trabajos relacionados:

\begin{itemize}[nosep]
  \item \textbf{Zaida} (IEEE Xplore, ACM Digital Library, Scopus, Google
    Scholar; b\'usqueda 2026-08-17):
    \texttt{"requirements elicitation" AND\allowbreak\ "systematic
    literature review"}
    \textbullet\ \texttt{"system usability scale" AND\allowbreak\ web
    application}
...
(\texttt{scripts/analisis-sus.py}, funci\'on \texttt{calcular\_puntaje\_sus()}).
...
\begin{itemize}[nosep]
  ...
  \item Resultado esperado de cada \emph{job}, verificado localmente en
    esta fase: \texttt{backend-test} (205 pruebas,
    \texttt{jacoco:\allowbreak check}
  ...
  \item Evidencia ZAP autenticada vigente (local TLS): 7 rutas protegidas
    verificadas en 2xx, 9 endpoints \'unicos, 0 High/Medium/Low, 2 tipos
    informativos (3 instancias). Baseline hist\'orico distinguido de la
    ejecuci\'on autenticada actual.
\end{itemize}

\section{Anexo E --- Checklist Ralph et al. (Engineering Research)}
\label{sec:anexo-ralph}

La tabla~\ref{tab:anexo-ralph} tabula de forma resumida el checklist
completo (\rutalarga{docs/checklists/ralph2021-engineering-research.md},
15~\'items):
...
E5 & Indica claramente la metodolog\'ia emp\'irica usada (DSR, secci\'on~\ref{sec:dsr-peffers}; GQM, secci\'on~\ref{sec:gqm}) & Cumple \\
...
La tabla~\ref{tab:anexo-matriz-completa} reproduce \'integramente
\texttt{docs/trazabilidad/matriz.csv} (44~requisitos), validada por
\texttt{scripts/validate-traceability.sh}. Columnas de endpoint y
evidencia emp\'irica cruda se omiten aqu\'i por espacio; est\'an
disponibles en el CSV original.

\begin{landscape}
\small
\begin{longtable}{@{}p{2.0cm}p{1.55cm}p{1.65cm}p{1.6cm}p{1.6cm}p{4.45cm}p{4.4cm}p{2.35cm}@{}}
...
REQ-F-003 & funcional & Must & HU-002 & CU-02 & \rutalarga{AuthController/JwtService} & \texttt{AuthControllerTest}\allowbreak+\texttt{JwtServiceTest} & verificado \\
...
REQ-F-006 & funcional & Must & HU-005 & CU-05 & \rutalarga{SecurityConfig/MascotaController} & \texttt{MascotaControllerTest} & implementado \\
...
REQ-F-013 & funcional & Should & HU-012 & CU-12 & \rutalarga{ConsultaController/ConsultaService} & \texttt{Consulta\allowbreak ControllerTest} (parcial) & implementado \\
...
REQ-F-020 & funcional & Should & HU-019 & CU-19 & \texttt{Authentication\allowbreak AuditService} & \texttt{Authentication\allowbreak AuditServiceTest} & implementado \\
REQ-F-021 & funcional & Should & HU-020 & CU-20 & \rutalarga{MascotaController/MascotaRepository} & \texttt{Resumen\allowbreak Especies\allowbreak IntegrationTest} & verificado \\
...
REQ-F-025 & funcional & Could & HU-024 & CU-24 & \rutalarga{ExternalApiController/ExternalApiService} & \texttt{External\allowbreak ApiServiceTest} (6 pruebas) & verificado \\
REQ-F-026 & funcional & Should & --- & --- & --- & Prevista (\texttt{MockMvc}); no implementada & pendiente \\
REQ-NF-001 & no funcional & Must & --- & --- & Cach\'e Redis (listado mascotas) & k6 v2.1.0 (10 corridas) & verificado \\
REQ-NF-002 & no funcional & Must & --- & --- & \rutalarga{SecurityConfig/Tomcat\allowbreak DualConnectorConfig} & \texttt{SecurityHeadersTest} & verificado \\
...
REQ-NF-005 & no funcional & Must & --- & --- & Frontend Angular (\texttt{styles.css}) & \texttt{Frontend\allowbreak ResponsivenessTest} (7 pruebas) & verificado \\
REQ-NF-006 & no funcional & Should & --- & --- & Frontend Angular & Manual (pendiente) & pendiente \\
REQ-NF-007 & no funcional & Must & --- & --- & \rutalarga{Makefile/docker-compose.yml} & \emph{healthcheck} Docker Compose & implementado \\
REQ-NF-008 & no funcional & Must & --- & --- & \texttt{SecurityConfig} (BCrypt) & \texttt{AuthControllerTest} & verificado \\
REQ-NF-009 & no funcional & Must & --- & --- & Logging de autenticaci\'on & \texttt{Authentication\allowbreak AuditServiceTest} & verificado \\
REQ-NF-010 & no funcional & Must & --- & --- & \texttt{LoginRate\allowbreak LimiterService} & \texttt{LoginRate\allowbreak LimiterServiceTest} & verificado \\
REQ-NF-011 & no funcional & Must & --- & --- & Estructura de paquetes backend & Revisi\'on de c\'odigo & verificado \\
REQ-NF-012 & no funcional & Should & --- & --- & Cach\'e Redis (TTL configurable) & Inspecci\'on + medici\'on Redis & implementado \\
REQ-NF-013 & no funcional & Must & --- & --- & \texttt{MascotaRepository} (\texttt{fn\_resumen\_...}) & \texttt{Resumen\allowbreak Especies\allowbreak IntegrationTest} & verificado \\
REQ-NF-014 & no funcional & Must & --- & --- & \rutalarga{TokenBlacklistService/JwtAuthenticationFilter} (cach\'e Redis) & Prevista; no implementada & pendiente \\
REQ-NF-015 & no funcional & Must & --- & --- & \rutalarga{MascotaService/CitaService/VacunaService/ConsultaService} & \texttt{MascotaControllerTest}\allowbreak+\texttt{CitaControllerTest}\allowbreak+\texttt{VacunaControllerTest}\allowbreak+\texttt{ConsultaControllerTest} & implementado \\
REQ-NF-016 & no funcional & Must & --- & --- & \rutalarga{RegistroRequest/UsuarioRequest/UsuarioService} & \texttt{AuthControllerTest}\allowbreak+\texttt{UsuarioControllerTest} & implementado \\
REQ-NF-017 & no funcional & Should & --- & --- & Procedimiento operativo (\texttt{pg\_dump}/\texttt{psql}) & Inspecci\'on documental + restauraci\'on real (no automatizada) & implementado \\
REQ-NF-018 & no funcional & Should & --- & --- & Frontend Angular (\texttt{login}/\texttt{mascotas}) & Lighthouse CI (\emph{Accessibility} $\geq$ 0{,}90) & verificado \\
\end{longtable}
\end{landscape}
...
las 41 referencias del informe (tabla~\ref{tab:anexo-referencias-impacto}),
\textbf{16 cumplen verificablemente el criterio de alto impacto} definido
arriba, por debajo del m\'inimo de 20 exigido por la Gu\'ia Final. No se
reclasificaron revistas regionales o peque\~nas como ``alto impacto''
\'unicamente para alcanzar el n\'umero
