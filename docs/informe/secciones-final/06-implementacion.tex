...
contra el despliegue p\'ublico real.

% EVIDENCIA-ZAIDA-01
% Ruta esperada: figuras/zaida/01-frontend-login.png
\evidencia{figuras/zaida/01-frontend-login.png}%
{BIOPET frontend login screen (production deployment, v1.0.0). UI text remains Spanish: the product is not internationalized, only report labels/diagrams are translated per the recalificaci\'on plan.}%
{fig:frontend-login-final}

% EVIDENCIA-ZAIDA-02
% Ruta esperada: figuras/zaida/02-crud-mascotas.png
\evidencia{figuras/zaida/02-crud-mascotas.png}%
{Pet CRUD in the frontend: list, create and edit (production deployment, v1.0.0).}%
