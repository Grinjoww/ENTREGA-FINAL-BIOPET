...
CRUD completo (\texttt{GET}\allowbreak/\texttt{POST}\allowbreak/\texttt{PUT}\allowbreak/\texttt{DELETE}
sobre \rutalarga{/api/mascotas} y \rutalarga{/api/mascotas/\{id\}}), con
...
\texttt{ADMIN} o \texttt{VETERINARIO} y est\'e activo, lanzando
...
\begin{lstlisting}[language=Java,caption={Acceso JPA formal a procedimientos almacenados en \texttt{Procedimiento\allowbreak BiopetRepository.\allowbreak java}.},label={lst:procedure-jpa}]
...
con \texttt{spring.\allowbreak flyway.\allowbreak baseline-on-migrate}, enmascaraba la migraci\'on
...
l\'inea con WCAG~2.1~\cite{wcag21}. Las figuras~\ref{fig:frontend-login-final}
y~\ref{fig:crud-mascotas-final} muestran el login y el CRUD de mascotas
del artefacto evaluado (v1.0.0): formulario \texttt{Iniciar sesi\'on} con
campos de correo y contrase\~na, y listado con resumen por especie,
paginaci\'on y acciones de edici\'on y borrado.

% EVIDENCIA-ZAIDA-01
% Ruta esperada: figuras/zaida/01-frontend-login.png
\evidencia{figuras/zaida/01-frontend-login.png}%
{Frontend login form of the evaluated artefact (v1.0.0).}%
{fig:frontend-login-final}

% EVIDENCIA-ZAIDA-02
% Ruta esperada: figuras/zaida/02-crud-mascotas.png
\evidencia{figuras/zaida/02-crud-mascotas.png}%
{Pet CRUD of the evaluated artefact (v1.0.0): list with summary by species and edit/delete actions.}%
