...
\begin{enumerate}[nosep]
  ...
  \item Evidencia emp\'irica multim\'etrica real: 205 pruebas
    ...
    0{,}0\,\% de tasa de error, usabilidad SUS evaluada sobre 18 registros
    anonimizados P01--P18, correspondientes seg\'un declaraci\'on del
    equipo a participantes reales, y auditor\'ia de seguridad por tres
...
cap\'itulo~\ref{cap:despliegue}. Las preguntas de investigaci\'on se responden
expl\'icitamente en la secci\'on~\ref{sec:respuestas-rq}. Ninguna cifra se estim\'o ni se
...
heredados de la Entrega~1A permanecen pendientes. El tag Git
\texttt{v1.0.0} ya fue creado y publicado originalmente sobre el commit
hist\'orico \texttt{0d5cd52} (2026-08-18); conforme a la r\'ubrica de esta
recalificaci\'on, ese mismo tag se actualiza para identificar el commit
final evaluado al cierre, sin cambiar su nombre
(secci\'on~\ref{sec:pendientes-honestos-publicacion}). El GitHub Release
correspondiente --- artefacto
distinto del tag Git --- todav\'ia no ha sido publicado a la fecha de
...
\begin{itemize}[nosep]
  ...
  \item Publicar un GitHub Release asociado al tag \texttt{v1.0.0} (ya
    creado sobre el commit \texttt{0d5cd52}), lo que activar\'a
    autom\'aticamente la publicaci\'on de las etiquetas
    \texttt{1.0.0}/\texttt{latest} en GHCR.
\end{itemize}
