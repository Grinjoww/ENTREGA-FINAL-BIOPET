...
an\'alisis din\'amico (OWASP ZAP Baseline, secci\'on~\ref{sec:zap}) y
an\'alisis est\'atico (SpotBugs + Find Security Bugs,
secci\'on~\ref{sec:spotbugs}), m\'as la auditor\'ia dedicada de SQL
...
\texttt{HttpOnly}\allowbreak+\texttt{Secure}\allowbreak+\texttt{SameSite=Strict}
...
cinco cabeceras en todas las respuestas (figura~\ref{fig:security-headers-final}):

\begin{itemize}[nosep]
  \item \texttt{X-Frame-Options: DENY}
  \item \texttt{X-Content-Type-Options: nosniff}
  \item \texttt{Content-Security-Policy: default-src 'self';
    frame-ancestors 'none'; object-src 'none'}
  \item \texttt{Referrer-Policy: no-referrer}
  \item \texttt{Strict-Transport-Security} solo sobre HTTPS
\end{itemize}

TLS~1.3 verificado en vivo
...
\begin{table}[H]
  \centering
  \caption[Resumen del an\'alisis est\'atico (SpotBugs).]{Resumen del an\'alisis est\'atico (fuente: \rutalarga{docs/mediciones/sec/static-analysis/README.md}).}
...
backend real, dentro de la red Docker Compose (\texttt{http://\allowbreak backend:8080},
\textbf{sin TLS}, \textbf{sin autenticaci\'on}), el
\texttt{2026-08-18T\allowbreak 02:53:21Z} (fuente:
\texttt{docs/mediciones/sec/zap/RUN-METADATA.txt}).

\begin{table}[H]
  \centering
  \caption[Resumen ZAP Baseline hist\'orico.]{Resumen ZAP Baseline hist\'orico (fuente: \texttt{docs/mediciones/sec/zap/README.md}).}
  \label{tab:zap-final}
  \begin{tabular}{@{}lr@{}}
    \toprule
    Severidad & Alertas \\
...
(\texttt{docker-compose.tls.yml}, \texttt{https://\allowbreak backend:8443}), el
\texttt{2026-09-03T\allowbreak 21:38:48Z} (fuente:
...
protegidas objetivo (\rutalarga{/api/usuarios/me}, \rutalarga{/api/mascotas},
\rutalarga{/api/mascotas/resumen-especies}, \rutalarga{/api/citas},
\rutalarga{/api/consultas}, \rutalarga{/api/vacunas}, \rutalarga{/api/usuarios})
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \midrule
    Target & \texttt{http://\allowbreak backend:8080} & \texttt{https://\allowbreak backend:8443} \\
...
\rutalarga{docs/mediciones/sec/zap/}:

\begin{itemize}[nosep]
  \item \texttt{zap-authenticated-local-report.json}, \texttt{.html}, \texttt{.xml}
  \item \texttt{RUN-METADATA-AUTH-LOCAL.txt}
  \item \texttt{zap-authenticated-local.yaml}
  \item \texttt{SHA256SUMS-AUTH-LOCAL.txt}
\end{itemize}

y en \rutalarga{scripts/run-zap-authenticated-local.sh} (script reproducible,
