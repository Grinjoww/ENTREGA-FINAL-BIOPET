...
\begin{itemize}[nosep]
  \item \textbf{REQ-F-023 --- Gesti\'on administrativa de usuarios}
    (\rutalarga{UsuarioController/UsuarioService}): un
  ...
  \item \textbf{REQ-F-025 --- Consulta de informaci\'on externa de especies}
    (\texttt{External\allowbreak ApiController}): consulta de datos de una especie
...
este informe salvo lo explicitado; la tabla~\ref{tab:req-pendientes} los
resume:
...
La tabla~\ref{tab:req-nf-relevantes} resume el estado de los REQ-NF
citados a lo largo de este informe, con la secci\'on que ampl\'ia cada
uno cuando corresponde:
