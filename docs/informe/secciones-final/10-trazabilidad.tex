...
frente al SRS. La tabla~\ref{tab:trazabilidad-final} selecciona las filas
m\'as representativas de las tres \'areas de trabajo del equipo, con el
estado reportado tal como lo declara el SRS/matriz vigente --- ning\'un
estado se marca \emph{verificado} sin una prueba automatizada o un
documento de evidencia citado.
...
REQ-F-006 & Jaime & Autorizaci\'on por rol (\texttt{@PreAuthorize}); aislamiento por propietario incompleto en consultas (v.\ REQ-NF-015) & \rutalarga{SecurityConfig/MascotaController} & \texttt{MascotaControllerTest}; \rutalarga{A01-access-control.md} & Implementado \\
REQ-F-007 & Zaida\allowbreak/Jaime & Perfil de usuario autenticado & \texttt{UsuarioController} & Postman (\rutalarga{docs/postman/}) & Verificado \\
...
REQ-F-020 & Jaime & Auditor\'ia de operaciones (parcial: solo autenticaci\'on) & \texttt{Authentication\allowbreak AuditService} & \texttt{AuthenticationAuditServiceTest}; \rutalarga{A09-logging.md} & Implementado \\
...
REQ-F-025 & Equipo (Unidad IV) & Consulta de informaci\'on externa de especies (cach\'e Redis) & \rutalarga{ExternalApiController/ExternalApiService} & \texttt{ExternalApiServiceTest} (6 pruebas); evidencia manual cach\'e fr\'ia/caliente & Verificado \\
...
REQ-NF-002 & Jaime & Comunicaci\'on cifrada obligatoria (HTTPS/TLS) & \texttt{Tomcat\allowbreak DualConnectorConfig} & Verificaci\'on en vivo, TLSv1.3 (desarrollo) & Verificado \\
REQ-NF-005 & Zaida & Interfaz responsiva (\emph{viewport}, \emph{media queries}) & Frontend Angular (\texttt{styles.css}) & \texttt{FrontendResponsivenessTest} (7 pruebas) & Verificado \\
...
REQ-NF-009 & Jaime & Auditor\'ia estructurada \texttt{AUTH\_AUDIT} & \texttt{Authentication\allowbreak AuditService} & \texttt{AuthenticationAuditServiceTest}; \rutalarga{A09-logging.md} & Verificado \\
REQ-NF-010 & Jaime & Rate limiting de login & \texttt{LoginRate\allowbreak LimiterService} & \texttt{AuthControllerTest}; \rutalarga{A07-authentication.md} & Verificado \\
REQ-NF-012 & Fred & Cach\'e del listado de mascotas con TTL externo & Redis/Valkey, \texttt{application.yml} & TTL confirmado (\rutalarga{docs/mediciones/redis/}); sin hit ratio & Implementado \\
REQ-NF-013 & Fred\allowbreak/Jaime & Estrategia h\'ibrida de acceso a datos (JPA + procedimientos almacenados) & \rutalarga{db/procs/}, repositorios JPA & \texttt{ResumenEspeciesIntegrationTest}; ADR-007 & Verificado \\
\end{longtable}
\normalsize
\end{landscape}
...
prueba automatizada o un documento de evidencia citado en su columna. La
reproducci\'on \'integra de los 44 requisitos est\'a en el anexo~F
(secci\'on~\ref{sec:anexo-matriz-completa}).
