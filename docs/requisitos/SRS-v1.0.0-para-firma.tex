...
\newcommand{\ciudadfecha}{Quevedo, 7 de septiembre de 2026}
...
- \textbf{Verificación:} Test \texttt{UsuarioControllerTest} (prueba
\texttt{meSigueFuncionando} sobre \texttt{GET\ /api/usuarios/me});
inspección de \texttt{UsuarioController}. - \textbf{Trazabilidad:} →
HU-006 → CU-06 → \texttt{UsuarioController.me} →
...
\texttt{VacunaControllerTest} (9 pruebas); colección Postman
...
\textbf{Verificación:} k6, 5 corridas en caliente y 5 en frío sobre el
tag v1.0.0, \texttt{docs/mediciones/perf/} (archivos
\texttt{k6-20260903T*-local-tls-v1.0.0-*.json} y \texttt{REPORT.md}). -
\textbf{Estado:} verificado. \texttt{docs/mediciones/perf/REPORT.md}
(2026-09-03) registra p95 entre 6.04 ms y 18.56 ms según la corrida
(máximo 15.06 ms en caché caliente y 18.56 ms en caché fría), muy por
debajo de los umbrales de 200 ms (caché caliente) y 500 ms (caché fría)
exigidos por este requisito, con 0.0 \% de error en todas las corridas.
...
Se incorporan igualmente sin origen en la Entrega 1A, durante la Unidad
IV: \texttt{REQ-F-023} (gestión administrativa de usuarios),
\texttt{REQ-F-024} (gestión de vacunas) y \texttt{REQ-F-025} (consulta
externa de especies con caché). Ninguno de estos sustituye ni duplica un
identificador \texttt{RF-NN} original.
...
\textbf{Cierre 2026-09-07 --- sincronización SRS / matriz / código antes
de la firma.}

\begin{itemize}
\tightlist
\item
  \textbf{REQ-F-007.} La verificación pasa a citar el test automatizado
  real (\texttt{UsuarioControllerTest.meSigueFuncionando},
  \texttt{GET\ /api/usuarios/me}), que la matriz ya registraba; se
  retira la afirmación ``sin test automatizado formal registrado'', que
  quedó desactualizada.
\item
  \textbf{REQ-F-024.} Corrección de conteo:
  \texttt{VacunaControllerTest} tiene 9 pruebas, no 10.
\item
  \textbf{Sección 4.} Se agregan a la tabla de correspondencia REQ-F-023
  (HU-022/CU-22), REQ-F-024 (HU-023/CU-23) y REQ-F-025 (HU-024/CU-24),
  que faltaban, y a la lista de requisitos sin origen en la Entrega 1A.
  La sección 4.1 pasa REQ-F-013 y REQ-F-015 de ``pendiente'' a
  ``parcial'', en línea con su estado en la sección 3.1 y en la matriz.
\item
  \textbf{REQ-NF-001.} Evidencia actualizada a la corrida oficial v1.0.0
  (2026-09-03): p95 entre 6.04 ms y 18.56 ms, archivos
  \texttt{k6-20260903T*}.
\item
  \textbf{Matriz de trazabilidad.} Estados y notas alineados con el SRS
  para REQ-F-013, REQ-F-015, REQ-F-020, REQ-F-025, REQ-NF-001 y
  REQ-NF-012. Detalle fila por fila en
  \texttt{docs/requisitos/CHANGELOG-REQ.md}.
\end{itemize}
