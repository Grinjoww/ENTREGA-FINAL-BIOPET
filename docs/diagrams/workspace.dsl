    ...
    model {
        ...
        biopet = softwareSystem "BIOPET" "Centralizes pet registration and role-based management (RBAC + JWT) for a veterinary clinic." {
            ...
            backend = container "Backend API" "Exposes the REST API: authentication, pets, appointments, consultations, vaccines, dashboard reports and the external-species proxy." "Spring Boot 3.2, Java 21" {
                ...
                petController      = component "PetController"      "CRUD and search endpoints for pets."                          "Spring MVC REST Controller"
                userController      = component "UserController"      "Exposes the authenticated user's profile."                    "Spring MVC REST Controller"
                appointmentController         = component "AppointmentController"         "CRUD and bulk status update for appointments."                "Spring MVC REST Controller"
                consultationController     = component "ConsultationController"     "Registers and queries clinical consultations."                "Spring MVC REST Controller"
                vaccineController       = component "VaccineController"       "CRUD for vaccine records."                                    "Spring MVC REST Controller"
                ...
                petService      = component "PetService"      "Pet business rules, ownership checks and the read cache."                    "Spring Service"
                appointmentService         = component "AppointmentService"         "Appointment business rules, scheduling and bulk status updates."             "Spring Service"
                consultationService     = component "ConsultationService"     "Clinical-consultation validation (active pet and veterinarian)."             "Spring Service"
                vaccineService       = component "VaccineService"       "Vaccine record business rules."                                              "Spring Service"
                ...
                userRepo      = component "UserRepository"             "User persistence."                                                                 "Spring Data JPA Repository"
                petRepo      = component "PetRepository"             "Pet persistence."                                                                  "Spring Data JPA Repository"
                appointmentRepo         = component "AppointmentRepository"                "Appointment persistence."                                                          "Spring Data JPA Repository"
                consultationRepo     = component "ConsultationRepository"            "Clinical-consultation persistence."                                                "Spring Data JPA Repository"
                vaccineRepo       = component "VaccineRepository"              "Vaccine-record persistence."                                                       "Spring Data JPA Repository"
                procedureRepo = component "BiopetProcedureRepository" "Formal JPA access (@Procedure / @NamedStoredProcedureQuery) to the 6 PostgreSQL stored procedures." "Spring Data JPA Repository"
        ...
        frontend -> petController "Calls" "HTTPS/JSON + cookie access_token"
        frontend -> userController "Calls" "HTTPS/JSON + cookie access_token"
        frontend -> appointmentController "Calls" "HTTPS/JSON + cookie access_token"
        frontend -> consultationController "Calls" "HTTPS/JSON + cookie access_token"
        frontend -> vaccineController "Calls" "HTTPS/JSON + cookie access_token"
        ...
        userController -> authService "Get profile (email)"
        ...
        petService -> redis "Cache reads/writes (@Cacheable/@CacheEvict)"
        ...
        procedureRepo -> postgres "Invokes stored procedures (@Procedure/@NamedStoredProcedureQuery)"
